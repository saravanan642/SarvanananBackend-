...
const createrUser = async(req ,res) =>{
    try{
        ...
        if(!name || !email || !password || !age || !phone || !isActive || !gender || !isActive || !profileImage || !skills || !marks){
            return res.josn({success : false , message :  "All filed are required"})
        }

        const isExiting = await Modeluser.findOne({email})
        if(isExiting) {
            return res.josn({success : false , message : "Already account exits please try again later"})
        }

        const phoneExixtig = await Modeluser.findOne({phone})
            if(phoneExixtig) {
                return res.josn(({success : false , message : "Already number exists plese try again later"}))
            }
            ...
            if(!save){
                return res.josn({success :false, message :"Failed to create please try again later" })
            }
